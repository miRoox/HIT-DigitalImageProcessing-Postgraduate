...
\begin{figure}[!htb]
    ...
    % -------------------------------------------------
    \node [term]            {输入图像};
    \node [proc, join]      {计算灰度直方图$p_r(r_k)$};
    \node [proc, join]      {计算累积分布$C(r_k)=\sum_{j=0}^k p_r(r_j)$};
    \node [proc, join]      {得到均衡化变换$T(r_k)=(L-1)\operatorname{round}[C(r_k)]$};
    \node [proc, join]      {遍历图像的每个像素$r_{x,y}$，\\得到输出图像的像素值$s_{x,y}=T(r_{x,y})$};
    \node [term, join]      {输出均衡化图像};
    \end{tikzpicture}
    \caption{直方图均衡化算法流程图}
